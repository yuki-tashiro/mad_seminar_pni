...
def get_args():
    ...
    parser.add_argument('--dataset_category', choices=['MVTec', 'BTAD', 'UAD'], default='MVTec', help="select type of dataset") # MVTec
    parser.add_argument('--category', default='hazelnut') # for UAD category: artefacts, for BTAD, category is ["01, "02", 03"]
...
if __name__ == '__main__':
    ...
    elif args.dataset_category == "UAD" :
        args.embedding_dir_path = os.path.join('./', f'embeddings_{"+".join(args.layer_index)}', args.category, args.backbone)
    ...
    if args.train_coreset :
        ...
        print("coreset_generator_trainer完了")
        coreset_generator = Coreset(args)
        print("coreset_generator完了")
    ...
    if args.train_nb_dist:
        ...
        distribution_trainer = pl.Trainer.from_argparse_args(args, max_epochs=args.num_epochs, gpus=1, logger=tb_logger, log_every_n_steps=1, enable_checkpointing=False) #log_every_n_steps=50 → 1に変更しよう
