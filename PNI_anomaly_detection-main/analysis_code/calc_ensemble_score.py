...
def get_args():
    ...
    parser.add_argument('--category', default='hazelnut') # hazelnut
...
if __name__ == '__main__':
    ...
    for idx, backbone in enumerate(args.backbone_list) : 
        ...
        img_pkl_list = sorted(glob.glob(os.path.join(default_root_dir, latest_version, "sample", '*_20?_???????.pkl'))) # 2つ書くと2度検索されるため消そう！
    ...
    for idx, fname in enumerate(fname_list) : # fname_listに同じpathがあるので注意
        
        fname = fname.split("/")[-1] # 必要．理由は，画像のラベルに
        fnum = fname.split("_")[-1] # これも必要．　元々でも使用されているため
        ftype = fname.strip(f"_{fnum}")
        print(f"fname: {fname}, fnum: {fnum}, ftype: {ftype}")
        ...
        print(f'{idx}, {fname}, ディレクトリ作成完了')
        print(len(gt_np))  # gt_np のサイズを確認
