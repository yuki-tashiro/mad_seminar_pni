...
if __name__ == '__main__':
    ...
    category_border = [0, 1, 13, 27, 33, 36, 53, 54, 70, 71, 90, 104, 132, 140, 142, 145, 150]
    # category_border = [0, 1, 17, 32, 39, 43, 61, 62, 81, 82, 104, 126, 170, 180, 182, 187, 192]
    # category_border = [0, 83, 233, 365, 482, 560, 670, 794, 909, 1076, 1236, 1353, 1395, 1495, 1574, 1725]
    category_list = ['absent_septum', 'artefacts', 'craniatomy', 'dural', 'ea_mass', 'edema', 'encephalomalacia', 'enlarged_ventricles', 'intraventricular', 'lesions', 'mass', 'posttreatment', 'resection', 'sinus', 'wml', 'other']
    ...
    # add
    pxl_precision_score_list = []
    pxl_recall_score_list = []
    y_true_rocauc = []
    y_score_rocauc = []
    ...
    for idx in range(len(category_list)) :
        ...
        # # category_amap_np がゼロサイズの配列でないことを確認
        # if category_amap_np.size > 0:
        ...
        y_true_rocauc.append(category_pxl_gt_np.ravel())
        y_score_rocauc.append(ensemble_pxl_score.ravel())
        print(f'Category : {category_list[idx]}, y_true: {category_pxl_gt_np.ravel().shape}, y_score: {ensemble_pxl_score.ravel().shape}, category_pxl_auc: {category_pxl_auc}')
        # category_img_auc = roc_auc_score(category_img_gt_np, ensemble_img_score) # image: ValueError: Only one class present in y_true. ROC AUC score is not defined in that case.
        # print(category_pxl_auc)
        ...
        # precision, recall リスト
        pxl_precision_score_list.append(precision)
        pxl_recall_score_list.append(recall)

        if args.calc_pro :

            category_pxl_pro = auc(recall, precision)
        ...
        # print("Pixel precision: ", f'{precision : .6f}') # errorの温床
        # print("Pixel recall: ", f'{recall : .6f}')
        if args.calc_pro :
            print("Pixel AUPRO: ", f'{category_pxl_pro : .6f}')
        print("")
        
        f = open(os.path.join(args.project_root_path, f"score_result.csv"), "a")
        data = [category_list[idx], 
        # str(f'{category_img_auc : .6f}'), 
        str(f'{category_pxl_auc : .6f}'), 
        str(f'{best_threshold : .6f}')
        ]
        if args.calc_pro :
        ...
        f.close()
        # else:
        #     # category_amap_np がゼロサイズの場合の処理
        #     print(f'Category: {idx}')
        #     print("No data available for ensemble pixel score calculations.")
        
    # avg_img_auc = np.mean(img_auc_score_list)
    avg_pxl_auc = np.mean(pxl_auc_score_list)
    # avg_pxl_precision = np.mean(pxl_precision_score_list)
    # avg_pxl_recall = np.mean(pxl_recall_score_list)
    if args.calc_pro :
        avg_pxl_pro = np.mean(pxl_pro_score_list)
    
    f = open(os.path.join(args.project_root_path, f"score_result.csv"), "a")
    data = ["Avg", 
    # str(f'{avg_img_auc : .6f}'), 
    str(f'{avg_pxl_auc : .6f}'),
    # str(f'{avg_pxl_precision : .6f}'),
    # str(f'{avg_pxl_recall : .6f}')
    ]
    ...
    # print(f'y_true_rocauc: {np.array(itertools.chain.from_iterable(y_true_rocauc)).shape}, y_score_rocauc:  {np.array(itertools.chain.from_iterable(y_score_rocauc)).shape}{y_score_rocauc}')
    # y_true_rocauc
    # y_score_rocauc
    # print(y_true_rocauc)
    np_y_true_rocauc = np.array(list(itertools.chain.from_iterable(y_true_rocauc)))
    np_y_score_rocauc = np.array(list(itertools.chain.from_iterable(y_score_rocauc)))
    print(f'{np_y_true_rocauc.shape}', '{np_y_score_rocauc.shape}')
    np.save('/content/drive/MyDrive/TUM/Unsupervised_Anomaly_Segmentation_in_Medical_Imaging_YU_TEAM/y_true_rocauc.npy', np_y_true_rocauc)
    np.save('/content/drive/MyDrive/TUM/Unsupervised_Anomaly_Segmentation_in_Medical_Imaging_YU_TEAM/y_score_rocauc.npy', np_y_score_rocauc)
