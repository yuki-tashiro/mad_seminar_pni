...
if __name__ == '__main__':
    ...
    for category in category_list :
        ...
        # type_list = os.listdir(amap_dir) # 1つしかないため無視せよ！
        type_list = sorted(os.listdir(amap_dir)) # 1つしかないため無視せよ！

        if not os.path.exists(amap_dir):
            print(f"Directory not found: {amap_dir}")
            continue  # このカテゴリーをスキップ
